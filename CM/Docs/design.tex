...
its waiting for the other list, it is already on its way to the proper node.
...
The design of the lock mechanism in the FineGrainTree is comparable with that
of the FineGrainList. The next node visited is locked before the lock on the
current node can be released. The difference is that it locks one of the child
nodes next instead of the single next node.
...
a linked list. All the advantages of the tree are then lost.

For the best performance, the number of threads should be a lot smaller than
the number of elements in the data structure. This means that there are
enough subtrees and therefore less interference between threads. Of course,
the head and first few levels of nodes can be expected to be the bottleneck
in this, since these will be visited by each thread on every operation.
Therefore the number of threads should not be too big. A maximum performance
can be expected after a certain number of threads.
